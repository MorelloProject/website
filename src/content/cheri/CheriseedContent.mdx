...
Program received signal SIGSEGV, Segmentation fault.
{{blue_c}}0x000000000023309a in {{yellow_c}}morello::shim::svc_impl () at src/svc.cpp:54
54      src/svc.cpp: No such file or directory.
(gdb) where -2
#15 0x00000000002785ff in printf (fmt=&lt;optimised out&gt;) at src/stdio/printf.c:9
#16 0x00000000002734c3 in {{yellow_c}}main () at after.c:43
...
The [fix] [https://git.morello-project.org/morello/android/vendor/arm/morello-examples/-/commit/146adba8e351c47069763286dd2d99e3931b64b6.diff]:
...
<CodeLayout>
  ...
  Let’s say we need the data of the string to be unmodifiable after we initialize
  them. CHERI compiler builtins are provided for accessing/modifying capability properties
  of pointers. For our use case, we can remove store permissions of the capability
  using cheri_perms_and() API.
  ...
  There are a set of builtin functions which provides access to capability properties.{" "}
  <a href="https://git.morello-project.org/morello/llvm-project/-/blob/cheriseed/clang/lib/Headers/cheriintrin.h">
    cheriintrin.h{" "}
  </a>
  provides interface to access and modify those properties including few
  capability permission constants. To learn more about this, see Sections 7 of
  the
